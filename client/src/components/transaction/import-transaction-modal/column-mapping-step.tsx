...
function ColumnMappingStep({
  ...
  const availableAttributes: AvailableAttributeType[] = useMemo(() => {
    return [{ fieldName: "Skip" }, ...transactionFields];
  }, [transactionFields]);
  ...
  const validMappingCount = Object.values(mappings).filter(
    (field) => field !== "Skip",
  ).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <Table>
          ...
          <TableBody>
            {csvColumns.map((column) => (
              <TableRow
                key={column.id}
                className={column.hasError ? "!bg-red-100" : ""}
              >
                ...
                <TableCell className="pl-8">
                  <div className="flex w-full items-center gap-0">
                    ...
                    <div className="w-[200px]">
                      ...
                      >
                        <SelectTrigger className="border-none shadow-none focus:ring-0 pl-2 w-full">
                          <SelectValue
                        ...
                        <SelectContent>
                          {availableAttributes.map((attr) => {
                            const isDisabled =
                              attr.fieldName !== "Skip" &&
                              mappings[column.name] !== attr.fieldName &&
      ...
      <div className="flex justify-between">
        ...
        >
          Continue ({validMappingCount} / {transactionFields.length})
